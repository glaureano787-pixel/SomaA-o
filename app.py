...
with tab_config:
    ...
    with col_input:
        ...
        if st.session_state.edit_index is not None:
            ...
            default_id, default_meta, default_qtd = lote_edit['id'], lote_edit['meta'], lote_edit['qtd']
                ...
                try:
                    ...
                    for lote in st.session_state.lista_lotes:
                        ...
                        if enc:
                            ...
                            # Se não encontrar, salvamos o erro no campo 'rolos' para exibir
        ...
        if st.session_state.resultados_atuais:
            ...
            for r in st.session_state.resultados_atuais:
                if r['status'] == "✅":
                    ...
                    st.markdown(f'<div class="resultado-balao-falha"><b>❌ Lote: {r["id"]}</b><br>Não encontrado. Faltam: {r["meta"]}kg</div>', unsafe_allow_html=True)
...
with tab_hist:
    for idx, op in enumerate(st.session_state.historico_operacoes):
        ...
        for d in op['detalhes']:
            msg = d['rolos'] if d['rolos'] else f"Faltaram {d['meta']}kg"
            st.write(f"{d['status']} {d['id']}: {msg}")
        st.markdown(f"<small>Sobra no pátio: {op['sobra']}</small>", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

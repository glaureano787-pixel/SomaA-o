...
st.markdown(f"""
    ...
    .btn-add>div>button {{ background: linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%); color: white; border: none; }}
    .btn-reset>div>button {{ background-color: #334155; color: #94a3b8; border: 1px solid #475569; }}
    .brand-title {{ font-size: 2.2rem; font-weight: 800; color: white; margin: 0; }}
    .resultado-balao {{ background-color: #3b82f6; padding: 15px; border-radius: 12px; color: white; margin-bottom: 10px; }}
    .resultado-balao-falha {{ background-color: #991b1b; padding: 15px; border-radius: 12px; color: #fecaca; margin-bottom: 10px; }}
    </style>
    """, unsafe_allow_html=True)

# 4. Lógica de Cálculo (Backtracking Otimizado)
...
# --- HEADER ---
st.markdown('<h1 class="brand-title">SOMA AÇO</h1><p style="color:#94a3b8; margin-bottom:30px;">SISTEMA DE CONFERÊNCIA</p>', unsafe_allow_html=True)
...
with tab_painel:
    ...
    with col_in:
        ...
        c_id = st.text_input("ID", key=f"id_{st.session_state.form_reset_key}")
        ...
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.subheader("Processar Carga")
        pesos_input = st.text_area("Lista de Pesos", height=100, placeholder="Ex: 2389, 2401, 2387...")
            ...
            if st.session_state.lista_lotes and pesos_input:
                try:
                    ...
                    st.session_state.historico_operacoes.insert(0, {
                        ...
                        "detalhes": resultado["lotes"],
                        "sobras": resultado["sobras"]
                    })
                    st.session_state.resultados_atuais = resultado
                    st.rerun()
                except: st.error("Use apenas números inteiros.")
        st.markdown('</div>', unsafe_allow_html=True)

        # BOTÃO LIMPAR TELA
        st.markdown('<div class="btn-reset">', unsafe_allow_html=True)
        if st.button("🧹 LIMPAR TELA (RESET)"):
        ...
        if st.session_state.resultados_atuais:
            ...
            for r in st.session_state.resultados_atuais['lotes']:
                if r['status'] == "✅":
                    ...
                    st.markdown(f'<div class="resultado-balao-falha"><b>❌ {r["id"]}</b>: Não encontrado.</div>', unsafe_allow_html=True)
...
with tab_hist:
    for op in st.session_state.historico_operacoes:
        with st.expander(f"Carga realizada às {op['hora']}"):
